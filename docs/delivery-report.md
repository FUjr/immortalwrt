...
`I2C_SD`（GPIO3）连接 RTC `SDA`，`I2C_SCLK`（GPIO4）连接 RTC `SCL`，
器件地址为 `0x51`。DTS 已启用控制器并声明 `nxp,pcf85063a`，镜像包含
`kmod-rtc-pcf85063`、`rtc-pcf85063.ko` 及自动加载配置；此前按 DS3231
地址 `0x68` 所作的假设和对应驱动已删除。

| 检查项 | 结果 | 说明 |
| --- | --- | --- |
| DTS 节点和固定地址 | PASS | 最终 DTB 中控制器为启用状态，RTC 地址为 `0x51` |
| 内核驱动和安装包 | PASS | 驱动编译为模块，正式镜像包含并自动加载 |
| I2C 控制器和管脚复用 | PASS | 两台实机均为 100 kHz，GPIO3/GPIO4 已分配给 I2C |
| 固件安装和启动 | PASS | 网关及下联板均完成镜像校验、NOR 写入并启动当前版本 |
| RTC 地址应答 | FAIL | 两台实机扫描 `0x51` 均无 ACK，驱动返回 `-145` |
| `/dev/rtc0`、时间读写和掉电保持 | BLOCKED | 器件未应答，尚不能进入功能测试 |

因此，RTC 的 DTS、驱动、构建和固件集成已经交付，但“硬件时钟在 NTP
不可用时提供本地时间源”仍为未通过项。下一轮硬件复验应先测量
`I2C_SD`/`I2C_SCLK` 空闲电平，确认上拉电阻、PCF85063AT 供电和接地，
检查两条信号的导通、短路及错接，再用示波器观察 `I2C_SCLK` 时钟和
`I2C_SD` 第九个时钟的 ACK。器件可应答后，继续验证 `hwclock` 读写、重启
恢复、断电保持及 NTP 不可用时的回退行为。
