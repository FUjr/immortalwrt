...
| PERF-001 | Traffic benchmark | PPS, throughput, loss, CPU, and latency are recorded without pass/fail thresholds | same-VRF external-endpoint throughput and PPS verified; MT7530 counters carried millions of packets while the CPU port saw only background/control traffic; NAT and latency matrices remain pending |
