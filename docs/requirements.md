...
| L2-008 | Same-VRF hardware switching | Multiple MT7530 member ports in one VRF share a DSA bridge, use a bridge MAC distinct from WAN, and offload same-subnet unicast in the switch chip | verified on LAN3-LAN1 at 94.2 Mbit/s forward and 87.5 Mbit/s reverse; minimum 64-byte Ethernet frames sustained 14,997 pps with zero loss, while switch counters increased by about 150K packets and CPU-port counters increased by only 52 RX/25 TX packets |
