...
| L2-008 | Same-VRF hardware switching | Multiple MT7530 member ports in one VRF share a DSA bridge, use a bridge MAC distinct from WAN, and offload same-subnet unicast in the switch chip | verified on LAN3-LAN1 at 94.2 Mbit/s forward and 87.5 Mbit/s reverse; hardware MIB counters carried over 100 MB per direction while the CPU port increased by only tens of KB, with zero port drops and CRC errors |
