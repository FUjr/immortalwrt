...
SEED="$ROOT/configs/misectel-7621evb-32m.config"
...
grep -qx 'CONFIG_TARGET_ramips_mt7621_DEVICE_misectel_7621evb-32m=y' .config || {
	echo '32M dual-slot device is not selected' >&2
	exit 1
}
env TMPDIR="$ROOT/tmp" make -j"$JOBS"

image="$(find "$OUTPUT" -maxdepth 1 -type f -name '*misectel_7621evb-32m-squashfs-sysupgrade.bin' -print -quit)"
[ -n "$image" ] || {
	echo '7621EVB 32M NAT sysupgrade image was not produced' >&2
	exit 1
}

manifest="$(find "$OUTPUT" -maxdepth 1 -type f -name '*misectel_7621evb-32m.manifest' -print -quit)"
[ -n "$manifest" ] || {
	echo '7621EVB 32M package manifest was not produced' >&2
	exit 1
}
grep -Eq '^uboot-envtools - ' "$manifest" || {
	echo 'uboot-envtools is missing from the 32M image manifest' >&2
	exit 1
}
...
size="$(wc -c < "$image")"
[ "$size" -le 16318464 ] || {
	echo "image exceeds the 15936 KiB firmware slot: $size bytes" >&2
	exit 1
}
...
uboot="$(find "$ROOT/build_dir" -type f \
	-path '*/u-boot-mt7621_misectel_7621evb-32m/u-boot-*/u-boot-mt7621.bin' \
	-not -path '*/.pkgdir/*' \
	-print -quit)"
[ -n "$uboot" ] || {
	echo '7621EVB 32M U-Boot image was not produced' >&2
	exit 1
}
...
uboot_output="$OUTPUT/immortalwrt-ramips-mt7621-misectel_7621evb-32m-u-boot.bin"
...
if [ -n "${BASE_MAC:-}" ]; then
	set -- --uboot "$uboot_output" --firmware "$image" \
		--base-mac "$BASE_MAC" \
		--output "$OUTPUT/immortalwrt-ramips-mt7621-misectel_7621evb-32m-programmer.bin"
	[ -z "${FACTORY_BIN:-}" ] || set -- "$@" --factory "$FACTORY_BIN"
	[ -z "${WOEM_BIN:-}" ] || set -- "$@" --woem "$WOEM_BIN"
	[ -z "${LEDEINFO_BIN:-}" ] || set -- "$@" --ledeinfo "$LEDEINFO_BIN"
	"$ROOT/scripts/pack-misectel-7621evb-32m-programmer.sh" "$@"
fi

set -- "$(basename "$uboot_output")" "$(basename "$image")"
programmer="immortalwrt-ramips-mt7621-misectel_7621evb-32m-programmer.bin"
[ ! -f "$OUTPUT/$programmer" ] || set -- "$@" "$programmer"
(cd "$OUTPUT" && sha256sum "$@") > \
	"$OUTPUT/immortalwrt-ramips-mt7621-misectel_7621evb-32m-artifacts.sha256"
